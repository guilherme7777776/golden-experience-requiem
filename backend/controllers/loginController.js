...
exports.abrirVisaoClienteConfirmacao = (req, res) => {
  console.log('loginController - Rota /login - visão do cliente - carrinho');
  res.sendFile(path.join(__dirname, '../../frontend/visaoCliente/confirmacao/confirmacao.html'));
...
exports.verificarSenha = async (req, res) => {
  ...
  const sqlFuncionario = `
    ...
    WHERE id_pessoa = $1 and c.id_cargo = f.id_cargo ;`;
  ...
  try {
    ...
    const resultCliente = await db.query(sqlCliente, [id_pessoa]);
...
exports.criarPessoa = async (req, res) => {
  ...
  try {
    const { id_pessoa, nome_pessoa, email_pessoa, senha_pessoa, data_nascimento } = req.body;
    ...
    const result = await db.query(
      'INSERT INTO pessoa (id_pessoa, nome_pessoa, email_pessoa, senha_pessoa, data_nascimento) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [id_pessoa, nome_pessoa, email_pessoa, senha_pessoa, data_nascimento]
    );
...
exports.atualizarSenha = async (req, res) => {
  try {
    ...
    const updateResult = await db.query(
      'UPDATE PESSOA SET senha_pessoa = $1 WHERE id_pessoa = $2 RETURNING id_pessoa, nome_pessoa, email_pessoa, data_nascimento',
      [nova_senha, id]
    );
